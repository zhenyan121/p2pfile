...
def main():
    ...
    try:
        ...
        # 更新启动画面状态
        splash.showMessage(
            f"正在初始化 {role} 角色...", Qt.AlignBottom | Qt.AlignCenter, Qt.white
        )
        app.processEvents()
        ...
        config_file = f"frpc_{role}.toml"
        config_path = os.path.join(config_dir, config_file)

        if not frp_connection.set_config_path(config_path):
        ...
        # 更新启动画面状态
        splash.showMessage(
            "正在连接到frp服务器...", Qt.AlignBottom | Qt.AlignCenter, Qt.white
        )
        app.processEvents()

        # 在单独的线程中启动frpc
        frp_thread = threading.Thread(target=lambda: frp_connection.start_frpc())
        ...
        # 创建主窗口
        window = MainWindow(frp_connection)
