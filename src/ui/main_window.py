...
class MainWindow(QMainWindow):
    ...
    def _init_ui(self):
        ...
        connection_frame.setStyleSheet(
            f"background-color: {COLORS['accent']}; border-radius: 5px;"
        )
        ...
        tabs = QTabWidget()
        tabs.setStyleSheet(
            f"QTabBar::tab:selected {{background-color: {COLORS['primary']}; color: white;}}"
        )
        ...
        file_frame.setStyleSheet(
            f"background-color: {COLORS['light']}; border-radius: 5px;"
        )
        ...
        self.select_file_btn.setStyleSheet(
            f"background-color: {COLORS['primary']}; color: white; padding: 8px;"
        )
        ...
        self.send_file_btn.setStyleSheet(
            f"background-color: {COLORS['highlight']}; color: {COLORS['text']}; padding: 8px;"
        )
        ...
        progress_frame.setStyleSheet(
            f"background-color: {COLORS['secondary']}; border-radius: 5px;"
        )
        ...
        self.progress_bar.setStyleSheet(
            f"QProgressBar {{border: 1px solid {COLORS['text']}; border-radius: 3px; text-align: center;}} "
        ...
        self.history_list.setStyleSheet(
            f"background-color: white; border: 1px solid {COLORS['text']};"
        )
    ...
    def _init_file_transfer(self):
        ...
        self.file_transfer = FileTransfer(
            host="127.0.0.1", port=local_port, callback=self.transfer_callback
        )
    ...
    def update_connection_status(self, status, extra_info=None):
        """
        更新连接状态
        ...
        # 处理连接建立
        if status in ["p2p_connected", "stcp_connected"] and extra_info:
            peer_addr = extra_info.get("peer_addr")
            peer_port = extra_info.get("peer_port")
            if peer_addr and peer_port:
                # 文件传输模块连接到对方
                if self.frp_connection.role == "visitor":
                    # 接收端需要连接到对方
                    self.file_transfer.connect(peer_addr, peer_port)
                else:
                    # 发送端等待对方连接，不需要额外操作
                    pass
    ...
    def update_status(self):
        ...
        if self.transfer_active and self.file_transfer:
            ...
            self.transferred_label.setText(
                f"已传输: {self._format_size(transferred)} / {self._format_size(total)}"
            )
    ...
    def send_file(self):
        ...
        # 开始传输
        threading.Thread(
            target=self._send_file_thread, args=(peer_addr, peer_port), daemon=True
        ).start()
    ...
    def transfer_callback(self, event, data):
        ...
        if event == "request":
            ...
            if accept:
                self.log(
                    f"接受文件传输请求: {file_name}, 大小: {self._format_size(file_size)}"
                )
    ...
    def change_download_path(self):
        ...
        dir_path = QFileDialog.getExistingDirectory(
            self, "选择下载目录", self.download_path_label.text()
        )
        if dir_path:
